...
\begin{DoxyItemize}
\item Instead of original 32-\/bit, C\+E\+LL is 16-\/bit, opcodes are 8-\/bit bytecode primitives.
\item To save space, primitives are compiled as bytecode and composite words are flagged address pointers.
\item For speed, use direct threading model with computed jump instead of original subroutine threaded,
...
\item Multi-\/tasking supported through interrupts. A base tick of 1ms precision using Timer2 i.\+e. one assigns mulply of 1ms as I\+SR repetition trigger period. For example 500 means triggered every 500ms and there are 8 timer interrupt handler slots provided. Timer1 is left free for Servo or other libraries.
\item On this 16-\/bit system, C\+L\+O\+CK still returns a double number (i.\+e. 32-\/bit) which takes 2 cells off stack. To calculate time difference, double arithmetic is needed, i.\+e. using D\+N\+E\+G\+A\+TE, D+, or D-\/ and the conversion words D$>$S, S$>$D.
