...
//
int tCNT = 0, tTAB = 0;                 // trace depth and indentation counter
...
void _nop() {}              // ( -- )
void _bye()                 // ( -- ) exit to OS, but nothing to do on a uC
{
    PRINTF(" reset...");
	R  = S = PC = IP = top = 0;
	WP = CELLSZ;
...
void _max0()                // (n1 n2 -- n) return greater of two top stack items
{
	if (top < STACK(S)) POP();
	else (U8)S--;
}
void _min0()                // (n1 n2 -- n) return smaller of two top stack items
{
	if (top < STACK(S)) (U8)S--;
	else POP();
}
