/**
 * @file
 * @brief eForth1 Interrupt handlers implementation
 * @note - replicated from nanoForth
 */
#include "eforth_intr.h"
///
/// eForth1 Interrupt handler -  static variables
///
namespace EF1Intr {
...
ISR(TIMER2_COMPA_vect) {
    ...
    cnt = 0;
    for (U8 i=0, b=1; i < EF1Intr::t_idx; i++, b<<=1) {
        digitalWrite(7, digitalRead(7) ? LOW : HIGH);  // DEBUG: ISR called
        if (++EF1Intr::t_cnt[i] < EF1Intr::t_max[i]) continue;
        EF1Intr::t_hit    |= b;
        EF1Intr::t_cnt[i]  = 0;
    }
}
ISR(PCINT0_vect) { EF1Intr::p_hit |= 1; }
ISR(PCINT1_vect) { EF1Intr::p_hit |= 2; }
ISR(PCINT2_vect) { EF1Intr::p_hit |= 4; }
#endif // ARDUINO
